...
from model.layer.message_passing_layer import MessagePassing
...
class GATConv(MessagePassing):
    ...
    def __init__(self,
        ...
        if new_node_feat:
            self.lin_ij = Linear(2 * out_channels, out_channels, bias=False)
        if isinstance(in_channels, int):
            if not double_input:
                self.lin_l = Linear(in_channels, heads * out_channels, bias=False)
            else:
                self.lin_l = Linear(2 * in_channels, heads * out_channels, bias=False)
        ...
        if bias and concat:
            ...
            self.bias = Parameter(torch.Tensor(heads * out_channels * 2))
    ...
    def message(self, x_i:Tensor, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        # newly added individual with x_i
    ...
    def aggregate(self, inputs: Tensor, index: Tensor,
        ...
        r"""Aggregates messages from neighbors as
                ...
                """
        #print(torch.cat((scatter(inputs, index, dim=self.node_dim, dim_size=dim_size, reduce='sum'), scatter(inputs, index, dim=self.node_dim, dim_size=dim_size, reduce='min')), dim = -1).size())
        return torch.cat((scatter(inputs, index, dim=self.node_dim, dim_size=dim_size, reduce='sum'),
                          scatter(inputs, index, dim=self.node_dim, dim_size=dim_size, reduce='min') +
